...
- [Sovereign Execution Engine](https://api.zuluworksai.com) - Headless operator for AI agents with 15 x402-protected execution primitives on Base (USDC). Cross-chain routing (XRPL/Hedera/Base), browser automation, EU AI Act compliance audit, web search, page extraction, DNS/SSL audit, KV storage, R2 storage, LLM inference, STT, text embeddings, image generation, cached LLM, browser automation, and code execution. $0.0005-$1.00 per call on Cloudflare Workers free tier. [MCP](https://api.zuluworksai.com/mcp)
